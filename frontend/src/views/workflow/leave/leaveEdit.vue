...
const flowCodeOptions = ref<{ value: string; label: string }[]>([]);

(async () => {
  const res = await listDefinition({ category: 103, pageNum: 1, pageSize: 1000 });
  flowCodeOptions.value = res.rows.map((item) => ({ value: item.flowCode, label: item.flowName }));
})();
